...
WAITLT:	CALL	GETCHR		;read a character
	CP	'U'		;check for user#
	JP	Z,SETUSER	;yes, go
	CP	':'		;check for start of hex
	JR	NZ,WAITLT	;wait for one or the other


	LD	C,DELF		;delete file specified on command line
	LD	DE,FCB		;point to default FCB
	CALL	BDOS

	LD	C,MAKEF		;create file specified on command line
	LD	DE,FCB		;default FCB again
	CALL	BDOS

GETHEX:
	CALL 	GETCHR		;now read a hex char
	CP	'>'		;check for end of file
	JR	Z,CLOSE		;go close if so
	LD   B,A		;else it's first char of hex
	PUSH BC
	CALL GETCHR		;get next char
	POP BC
	LD   C,A		;now BC is length in hex

	CALL BCTOA		;convert length

	LD	B,A		;length to B
	LD	A,(checkSum)	;else get checksum (zero to start)
	ADD	A,B		;and add to it
	LD	(checkSum),A	;store new checksum
	LD	A,(byteCount)	;get line byte count
	INC	A		;increment
	LD	(byteCount),A	;store new count

	LD	A,B		;get length

	LD	HL,(buffPtr)	;get buffer pointer

	LD	(HL),A		;store length in buffer
	INC	HL
	LD	(buffPtr),HL	;point past

	LD	A,(buffPos)	;increment byte count
	INC	A
	LD	(buffPos),A
	CP	80H
